...
import CommentApi from './commentApiMock.js';
...
export default class CommentPopup {
  ...
  loadPopup() {
    const showDetails = this.fetchShowDetails();
  ...
  commentCounter = () => {
    const comments = CommentApi.getComments(this.selectedItem);
  ...
  displayComments = () => {
    ...
    const countComment = this.commentCounter();
  ...
  submit() {
    ...
    CommentApi.postComment(comment.item_id, comment.username, comment.comment);
    this.displayComments();
